...
class ExportController extends Controller
{
    ...
    public function sql()
    {
        ...
        fwrite($handle, "-- GoWithDog database export ({$timestamp})\n");
        ...
        $zip = new ZipArchive;
        $zip->open($zipPath, ZipArchive::OVERWRITE);
        $zip->addFile($sqlPath, "gowithdog-{$timestamp}.sql");
        $zip->close();
        @unlink($sqlPath);

        return response()->download($zipPath, "gowithdog-db-{$timestamp}.zip")->deleteFileAfterSend(true);
    }
