...
- `docker-compose.prod.yml` is a merge overlay (uses Compose's `!override`/`!reset` YAML tags) — it switches the API image to its `prod` build target (immutable, no bind mounts, optimized autoloader/opcache) and shares its `public/`/`storage` tree with `api-nginx` via a named volume (`api_html`) instead of a bind mount, since the prod image has no host mount. If you rebuild the api image with new static assets, `docker volume rm gowithdog_api_html` to force nginx to pick them up. The `web` service has no prod override — the front-end is deployed separately on Vercel (own build/CDN/deploys, decoupled from the Pi), not run from this compose file in production; `woofalk-app/Dockerfile`'s `prod` target still exists standalone if self-hosting the front is ever needed again. (This volume name — and the underlying `COMPOSE_PROJECT_NAME=gowithdog` in `scripts/deploy-pi.sh` — intentionally still says "gowithdog", not "woofalk": it's an internal Compose project identifier on the live Raspberry Pi deployment, and renaming it would make Compose treat `api`/`api-nginx` as belonging to a different project than the already-running `db`/`mailhog`, risking lost network connectivity and an orphaned/reinitialized database volume. Not worth it for an invisible technical identifier.)
...
### Architecture
- Routing is defined centrally in `src/index.jsx` (not `App.jsx`) — all top-level routes and their admin/auth gating live there via `react-router`'s `<Routes>`. `App.jsx` just renders the shared `<Outlet/>` layout inside `index.jsx`'s tree. Depend on the unified `react-router` package (not `react-router-dom`): v8 discontinued `react-router-dom` (it never published past 7.18.x) and folded its DOM exports — `BrowserRouter`, `Link`, `useNavigate`, etc. — into `react-router` itself, fixing GHSA-qwww-vcr4-c8h2 (a CSRF issue in RSC/framework mode, not used here, but only patched in the unified v8 line). All `react-router-dom` imports were mechanically swapped to `react-router` during that migration; don't reintroduce `react-router-dom` as a dependency.
- The API origin is centralized in `src/config.jsx` (`export const API_URL = import.meta.env.VITE_API_URL || "https://api.woofalk.com"`) and imported wherever a request is made. It used to be hardcoded to the production domain (formerly `https://api.gowithdog.fr`, then briefly `https://api.woofalk.fr`, now `https://api.woofalk.com`) in ~50 places across every component/service file, which meant local/Docker dev always hit prod — if you add a new API call, import `API_URL` from `./config` (or the right relative path) rather than hardcoding a host.
